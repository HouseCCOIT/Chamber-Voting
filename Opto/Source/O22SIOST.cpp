...
int O22SnapIoStream::OpenStreaming(long nType, long nLength, long nPort)
//-------------------------------------------------------------------------------------------------
...
//
// Standard means that default stream message is being sent from the I/O units.
// Custom means that a custom memory map area and length is being streamed from the device.
// This class expects all devices to be streaming the same type and length.
//-------------------------------------------------------------------------------------------------
{
  ...
  // Initialize WinSock.dll
	...
	m_pbyLastStreamBlock = new uint8_t[m_nStreamLength];
  ...
  // Other systems may use ioctl() or fcntl()
	...
	// Setup the socket address structure for binding the UDP server (receiver) socket, IPv4
	// WARNING! This example address configuration opens the stream port on all of the computer's
	// network interfaces. This could open up a security hole. If this concerns you, resolve
	// the specific interface's IPv4 address and use it as the .sin_addr value. Then the
	// stream only opens on the specific interface.
	sockaddr_in StreamSocketAddress;
	StreamSocketAddress.sin_addr.s_addr = INADDR_ANY;
	StreamSocketAddress.sin_family = AF_INET;
	StreamSocketAddress.sin_port = htons((uint16_t)nPort);
...
int O22SnapIoStream::StartStreamListening(char * pchIpAddressArg, long nTimeoutMS)
//-------------------------------------------------------------------------------------------------
...
#ifdef _WIN32
		IN_ADDR inIpv4Addr;
		InetPton(AF_INET, (PCTSTR)pchIpAddressArg, (PVOID)&inIpv4Addr);
		pNewStreamItem->nIpAddress = inIpv4Addr.S_un.S_addr;
#elif defined _LINUX
		inet_pton(AF_INET, (const char *)pchIpAddressArg, (void *)&(pNewStreamItem->nIpAddress));
#else
		? ?
#endif
...
//-------------------------------------------------------------------------------------------------
{
	...
	IN_ADDR inIpv4Addr;
	InetPton(AF_INET, (PCTSTR)pchIpAddressArg, (PVOID)&inIpv4Addr);
	unsigned long nIpAddressArg = inIpv4Addr.S_un.S_addr;
#elif defined _LINUX
	struct in_addr inIpv4Addr;
	inet_pton(AF_INET, (const char *)pchIpAddressArg, (void *)&inIpv4Addr);
	unsigned long nIpAddressArg = inIpv4Addr.s_addr;
#else
	? ?
#endif
...
//-------------------------------------------------------------------------------------------------
{
	...
	if (0 == select((int)m_StreamSocket + 1, &fds, NULL, NULL, &tvStreamTimeOut))
	{
	...
	nResult = recvfrom(m_StreamSocket, (char*)m_pbyLastStreamBlock, m_nStreamLength, 0, (sockaddr*)&SourceAddress, &nSourceLen);
...
//-------------------------------------------------------------------------------------------------
{
	...
	// Flip-flop all the bytes so they're in the right order for the PC (assuming PC is little endian)
