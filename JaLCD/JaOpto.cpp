...
unsigned int JaOpto::voteBoardLights(unsigned int seat, unsigned int voteIn)
{
   unsigned int index;
...
bool JaOpto::voteOn(void)
{
   ...
   // Read Vote On switch
   bool latchOn = false;
   inOpto->ReadClearDigitalLatch64(41, true, latchOn);

   if (latchOn) {
      outOpto->WriteDigitalState64(44, true);
      voteOn = true;
   }
...
bool JaOpto::voteOff(void)
{
   ...
   // Read Vote Off switch
   bool latchOn = false;
   inOpto->ReadClearDigitalLatch64(42, true, latchOn);

   if (latchOn) {
      outOpto->WriteDigitalState64(44, false);
      voteOff = true;
   }
