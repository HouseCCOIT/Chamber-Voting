...
CListBoxCH::~CListBoxCH()
{
	...
	{
		CString* pItem = m_pItemList.GetHead();
		delete pItem;
...
void CListBoxCH::AddItem(LPCSTR lpszItemName)
{
	CString* pItem = new CString(lpszItemName);
...
void CListBoxCH::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	//
	//
	CString* pItem = static_cast<CString *>(GetItemDataPtr(lpDIS->itemID));
	...
	pDC->SetTextColor(RGB(0x80,0,0));			// unselected text color

	//	rect.CopyRect(rectText);
	//	rect.DeflateRect(2,2);
	//	rect.OffsetRect(2,10);
	if( pItem != NULL ) {
//			pDC->DrawText((LPCTSTR)pItem, lstrlen((LPCTSTR)pItem), rect, DT_LEFT | DT_SINGLELINE);
	}
