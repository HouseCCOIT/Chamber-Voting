...
unsigned int JaOpto::voteDeskLights(unsigned char seat, unsigned int voteIn)
{
...
unsigned int JaOpto::voteDeskLightsRemote(unsigned char seat, unsigned int voteIn)
{
   //CSingleLock OptoSem(&OptoMutex);
...
unsigned int JaOpto::voteBoardLights(unsigned char seat, unsigned int voteIn, unsigned int seat2)
{
   //CSingleLock OptoSem(&OptoMutex);
...
unsigned int JaOpto::voteBoardLightsRemote(unsigned char seat, unsigned int voteIn, unsigned int seat2)
{
   //CSingleLock OptoSem(&OptoMutex);
   ...
   switch (voteIn) {
   case 0: // Reset
      retCode1 = outOpto->WriteDigitalState64(index + 0, false);
      Sleep(1);
      retCode2 = outOpto->WriteDigitalState64(index + 1, false);
      break;

   case 1: // Nay 
      retCode1 = outOpto->WriteDigitalState64(index + 0, false);
      Sleep(1);
      retCode2 = outOpto->WriteDigitalState64(index + 1, true);
      break;

   case 2: // Yea
      retCode1 = outOpto->WriteDigitalState64(index + 0, true);
      Sleep(1);
      retCode2 = outOpto->WriteDigitalState64(index + 1, false);
      break;
   ...
   //if (retCode1 || retCode2) TRACE("retCode1, retCode2 = %d, %d", retCode1, retCode2);

   //TRACE("seat, vote = %d, %d", seat, voteIn);
...
unsigned int JaOpto::voteOpenCheck(bool resetLatch)
{
   ...
   //return(true); //TBD - TEST_ONLY if uncommented
