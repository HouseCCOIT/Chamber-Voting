...
CRVDlg::CRVDlg(CWnd* pParent /*=NULL*/)
   : CDialog(CRVDlg::IDD, pParent),
   m_FontName("Courier")
{
...
void CRVDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CRVDlg, CDialog)
...
const char * JaDispDat = "V:\\JaDisp2.dat";

/////////////////////////////////////////////////////////////////////////////
// JaDisplay message handlers

BOOL CRVDlg::OnInitDialog()
{
   CDialog::OnInitDialog();

   SetWindowPos(&CWnd::wndBottom, 0, 0, MAX_SCREEN_X, MAX_SCREEN_Y, 0); // SWP_NOMOVE | SWP_NOSIZE);
...
void CRVDlg::SetFont(CString fontName)
{
   ClearAll();
...
void CRVDlg::SetFontSize(int fontSize)
{
   ClearAll();
...
void CRVDlg::SetLine(int i, char *text)
{
...
void CRVDlg::SetResultLine(char *text1, char *text2, char *text3, char *text4)
{
...
BOOL CRVDlg::OnEraseBkgnd(CDC* pDC)
{
   CRect rc;
...
void CRVDlg::SetWaterMark(CString watermark)
{
   m_Watermark = CString("V:\\bitmapsTV\\") + watermark;
}

BOOL CRVDlg::LoadDispOptions()
{
   FILE *dispFile;
...
void CRVDlg::ShuttingDown()
{
   m_ShuttingDown = TRUE;
}


FILETIME CRVDlg::JaGetFileTime(CString fName)
{
   HANDLE hFile = NULL;
...
BOOL CRVDlg::DisplayFileChanged()
{
   BOOL retVal = false;
...
void CRVDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void CRVDlg::DisplayResults(BOOL DisplayOn)
{
